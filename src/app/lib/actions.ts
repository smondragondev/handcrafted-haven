...
export async function createProduct(
    ...
    if (image instanceof File && image.size > 0) {
        blob = await put("products/" + Date.now() + "-" + image.name, image, {
            access: "public",
        });
    }

    const formCategory =
        formData.get("category") === "new-category"
            ? formData.get("new-category")
    ...
    const { name, description, category, price, imageUrl, contributorId } =
